...
- **The SPA fallback is `not_found_handling: "single-page-application"`** in `wrangler.jsonc`, and nothing else. Without it, a refresh on any client route is a 404, because no such file exists. **A `_redirects` file must not be added alongside it** — the obvious SPA rule, `/* /index.html 200`, is rejected by the Workers assets API as an infinite loop, because Workers already strips `.html` and `/index` when matching, so the rewrite re-triggers itself. It failed a deploy. If the host ever changes, reintroduce `_redirects` then.
...
- **`EXPO_PUBLIC_*` values are build variables, not Worker variables.** A static-asset Worker has no script and never reads its own `vars`; Metro inlines these at export time. Setting them in the wrong place produces a build that succeeds and an app that cannot sign anybody in. `apps/mobile/scripts/check-env.mjs` now fails the build when they are set to something implausible — it does not fail when they are absent, because CI builds the app to prove it bundles and has no reason to hold credentials.
